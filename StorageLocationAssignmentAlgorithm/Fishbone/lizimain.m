...
for k=1:3
    ...
    particle_fitvalue=zeros(1,particle_num); %初始化每个粒子的最适应度
    bestfit_all=0; %初始化历史最适应度
    particle_best_generation=zeros(pop_length,goods_pos);%初始化每个粒子的历史最优个体
    bestparticle=zeros(goods_num,goods_pos);%初始化粒子群历史最优个体  
for q=1:generation   
...
pop_father=pop;%粒子群
objvalue_father=objvalue;%粒子群的各个粒子适应度
