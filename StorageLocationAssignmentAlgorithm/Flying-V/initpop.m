...
%           goods_common=0;   
 ...
 %%%%%%%%%%%%%%%%%%%%%%除去不能放的位置%%%%%%%%%%%%%%%%%%%%%%%%%
           if (pop_area(j)==1)||(pop_area(j)==2)           
               if pop((i-1)*goods_num+j,6)==0   %检查奇数排
                   if ((pop_list(j)>(1.5*pop_row(j)-0.5))||(pop_row(j)>10))
                   goods_death=1;
                   end
               else   
                   if ((pop_list(j)>(1.5*pop_row(j)))||(pop_row(j)>10))
                   goods_death=1;
                   end
               end
           else
               if pop((i-1)*goods_num+j,6)==0  
                  ...
                  if (pop_list(j)>(15-1.5*pop_row(j)))||(pop_row(j)>9)                       
                  goods_death=1;
                  end
               end  
           end  
