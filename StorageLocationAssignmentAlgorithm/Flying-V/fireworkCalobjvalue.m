function [objvalue,average_objvalue] =fireworkCalobjvalue(pop,goods_num,goods_Vv,goods_Vy,layer_height,layer_width,goods_frq,goods_weight,time_rate,gravity_rate,goods_gravity_optimal_value,goods_time_optimal_value,k)
%calobjvalue函数的功能是实现目标函数的计算
particle_num = length(pop)./goods_num;
objvalue_all=0;
...
%%%%%%%%%%%%%%%计算种群中每个物体的时间%%%%%%%%%%%%%%%
for i=1:pop_length
    if (pop_area(i)==1)||(pop_area(i)==2)            
        if   mod(pop_row(i),2)==1   %说明是奇数排
              pop_time_v(i)=sqrt(2)*(1+1.5*(pop_row(i)-1))*layer_width./(goods_Vv);%求出1区和2区，奇数排的pop_time_v 
        else   %说明是偶数排    
              pop_time_v(i)=(sqrt(2)*1.5*pop_row(i)+1)*layer_width./(goods_Vv);%求出1区和2区，偶数排的pop_time_v 
        end
    elseif(pop_area(i)==3)||(pop_area(i)==4) 
        if   mod(pop_row(i),2)==1   %说明是奇数排
              pop_time_v(i)=(sqrt(2)*1.5*(pop_row(i)-1)+2)*layer_width./(goods_Vv);%求出3区和4区，奇数排的pop_time_v 
        else             %说明是偶数排    
              pop_time_v(i)=(sqrt(2)*(1.5*pop_row(i)-1)+1)*layer_width./(goods_Vv);%求出3区和4区，偶数排的pop_time_v 
        end
    else
       disp('货区越界'); 
    end
    if (pop_layer(i)>4||pop_layer(i)<1)
        disp('货架层数越界');
    end
