...
%%%%%%%%%%%对死个体的个别位置进行变异。如果有死个体的话对死个体的所有不满足约束的位置进行变异，变成活个体，求其适应值。
%******************过滤掉不符合的个体*****************************%
 ...
 %%%%%%%%%%%%%%%%%%%%%%除去不能放的位置%%%%%%%%%%%%%%%%%%%%%%%%%
           ...
           if(area==1||area==2)
               if (mod(row,2)==1) %检查奇数排
                    if (list>1.5*row-0.5)||row>10||list<=0||row<=0
                    goods_death=1;
                   end
               else                          
                    if (list>1.5*row)|| row>10||list<=0||row<=0  
                    goods_death=1;
                    end
               end
           elseif(area==3||area==4)    
                if (mod(row,2)==1) %检查奇数排
                  ...
                  if (list>(15-1.5*row))||(row>9)||list<=0||row<=0                     
                  goods_death=1;
                  end
                end  
           else
